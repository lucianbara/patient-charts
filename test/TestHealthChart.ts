...
contract("HealthChart", ([deployer]) => {
    ...
    it("should create a second Doctor", async () => {
        let instance = await HealthChart.deployed();
        let doctor = await instance.addDoctor("doctordoctor4", "password1", "0xfCB55cBDb99d5b153D339C2076e423dbFa878353", "Doctor", "Strange");
    ...
    it("should add a patient", async () => {
        let instance = await HealthChart.deployed();
        let patneient = await instance.addPatient("1890000000000", "L", "B");
        let patient = await instance.getPatient(0);
        assert(patient[0] === "1890000000000");
        assert(patient[1] === "L");
        assert(patient[2] === "B");
        assert(patient[3] === "0xE806a93498d28A434a2A458fF61872981DA3F35E"); //First address in the block chain
    });

    it("should transfer the patient to the second doctor", async () => {
        let instance = await HealthChart.deployed();
        let patneientId = await instance.transferPatient(1, 0);
        let patient = await instance.getPatient(0);
        assert(patient[0] === "1890000000000");
        assert(patient[1] === "L");
        assert(patient[2] === "B");
        assert(patient[3] === "0xfCB55cBDb99d5b153D339C2076e423dbFa878353"); //First address in the block chain
    });

    it("Should add a health event for the first patient", async () => {
        let instance = await HealthChart.deployed();
        let eventId = await instance.addHealthEvent(0, "Broke leg while sking");
        let event = await instance.getEventsForPatient(0);
    ...
    it("Should add a health event for the first patient, by the second doctor", async () => {
        let instance = await HealthChart.deployed();
        let eventId = await instance.addHealthEvent(0, "Broke another leg while walking", {from: "0xfCB55cBDb99d5b153D339C2076e423dbFa878353"});
        let event = await instance.getEventsForPatient(0);
        assert(event[0].toString() === "0");
